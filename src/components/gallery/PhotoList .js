...
const PhotoItem = ({ info: { url } }) => {
  return (
    <div className="relative h-[300px] cursor-pointer group">
      ...
      <span
        className=" absolute right-5 top-5 z-10 cursor-pointer opacity-0 invisible 
      group-hover:opacity-100 group-hover:visible transition-all w-8"
      >
        <svg
          width="42"
          height="38"
      ...
      <button
        className="py-2 px-6 text-sm font-medium bg-white rounded-lg text-black absolute bottom-5 left-2/4 -translate-x-2/4 opacity-0 invisible 
